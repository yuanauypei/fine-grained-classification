...
progpath = "/home/zhuyuan/zy_all/datasets_all/stanford_cars/"
sys.path.append(progpath)
savepath = "/home/zhuyuan/zy_all/SEF-master/datasets/stcars/"

imdbpath = os.path.join(savepath, "imdb.pkl")

if not os.path.exists(imdbpath):
    annos_name = "cars_annos.mat"
    annos = sio.loadmat(os.path.join(progpath, annos_name),
    ...
    with open(os.path.join(savepath, 'imdb.pkl'), 'wb') as handle:
        pk.dump({'classdict': classdict, 'classnames': classnames,
                 'annos_test': annos_test, 'annos_train': annos_train}, handle)
else:
    with open(os.path.join(savepath, 'imdb.pkl'), 'rb') as handle:
